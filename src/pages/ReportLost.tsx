  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        const filePath = `lost/${user.id}/${Date.now()}.${fileExt}`;
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-strong rounded-3xl p-8 md:p-10">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <Input value={itemName} onChange={(e) => setItemName(e.target.value)} placeholder="e.g. Blue Backpack, iPhone 15, Water Bottle..." className="bg-secondary/50 border-border/30 h-11 rounded-xl" />
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <Input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Where was it lost?" className="bg-secondary/50 border-border/30 h-11 rounded-xl" />
              </div>
              <div className="space-y-2">
                <Label className="text-sm font-medium">Date Lost *</Label>
                <Input type="date" value={dateLost} onChange={(e) => setDateLost(e.target.value)} max={new Date().toISOString().split("T")[0]} className="bg-secondary/50 border-border/30 h-11 rounded-xl" />
              </div>
            </div>

            <div className="space-y-2">
              <Label className="text-sm font-medium">Description *</Label>
              <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe the item in detail — color, shape, material, any text/logos, condition, contents (if bag/wallet)..." className="bg-secondary/50 border-border/30 min-h-[140px] rounded-xl" />
            </div>
